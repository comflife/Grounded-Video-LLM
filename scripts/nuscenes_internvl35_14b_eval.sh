...
OUTPUT_DIR="${OUTPUT_DIR:-${EXP_ROOT}/eval_results/nuscenes_internvl35_14b}"
...
if [[ ! -d "${MODEL_PATH}" ]]; then
  ...
  echo "Run: bash scripts/download_internvl35_14b_weights.sh" >&2
  exit 1
fi
...
if [[ "${LOAD_IN_8BIT}" == "1" ]]; then
  EVAL_ARGS+=(--load_in_8bit)
fi

cd "${REPO_ROOT}"
"${PYTHON_BIN}" scripts/nuscenes_internvl35_14b_eval.py "${EVAL_ARGS[@]}"
