...
OUTPUT_DIR="${OUTPUT_DIR:-${EXP_ROOT}/eval_results/nuscenes_gemma4_12b_it_cot_prompt}"
BASELINE_METRICS="${BASELINE_METRICS:-${EXP_ROOT}/eval_results/nuscenes_gemma4_12b_it/eval_metrics.json}"
...
LIMIT="${LIMIT}" \
bash "${SCRIPT_DIR}/nuscenes_gemma4_12b_eval.sh"
...
if [[ -f "${BASELINE_METRICS}" ]]; then
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/compare_nuscenes_eval_metrics.py" \
    ...
    --baseline_name "gemma4_original_prompt" \
    --candidate_name "gemma4_cot_prompt"
fi
