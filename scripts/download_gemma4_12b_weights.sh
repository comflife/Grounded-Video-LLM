...
REPO_ID="${REPO_ID:-google/gemma-4-12B-it}"
MODEL_DIR="${MODEL_DIR:-${WEIGHT_PATH}/gemma-4-12b-it}"
PYTHON_BIN="${PYTHON_BIN:-${EXP_ROOT}/envs/gemma4-eval/bin/python}"

if [[ ! -x "${PYTHON_BIN}" ]]; then
  echo "Gemma 4 eval env not found at ${PYTHON_BIN}" >&2
  echo "Run first: bash scripts/setup_gemma4_eval_env.sh" >&2
  exit 1
fi
